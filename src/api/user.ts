...
export const signUp = async ( email: string,userName:string,age: number,gender: string,password: string,phoneNumber:string):Promise<response> => {
  const response = await Api.post(userRoutes.signUp,{email,userName,age,gender,password,phoneNumber})
  return response
};
...
export const verifyOtp=async(otp:number,email:string):Promise<verifyOtpResponse>=>{
