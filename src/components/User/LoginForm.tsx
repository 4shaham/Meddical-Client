...
function LoginForm() {
  ...
  const responseMessage = async(response:CredentialResponse):Promise<void>=> {
    ...
    try {
      let creditonalDetails:CredentialPayload = jwtDecode(response.credential as string);
      console.log(creditonalDetails.email, "response");
    let ApiResponse=await googleAuth(creditonalDetails.email,creditonalDetails.name,creditonalDetails.picture)
